...
class MLPCVTrainer:
    ...
    def fit_one_fold(self, tr_df, fold=0):
        ...
        y_tr_tensor = torch.tensor(
            y_tr, dtype=torch.float32, device=self.device
        )
        w_tr_tensor = torch.tensor(
            w_tr, dtype=torch.float32, device=self.device
        )
        ...
        for epoch in range(self.epochs):
            ...
            for xb, yb, wb in train_loader:
                optimizer.zero_grad()
                with autocast():  # ← FP16でforward・loss計算
                    preds = model(xb)
                    loss = criterion(preds, yb)
                    loss = (loss * wb).mean()
            ...
            with torch.no_grad():
                for xb, yb in val_loader:
                    xb = xb.to(self.device)
                    with autocast():  # evalでもautocast可（省略可能）
                        pred = model(xb).cpu().numpy()
                    preds.append(pred)
            val_pred = np.concatenate(preds)
            val_rmse = np.sqrt(mean_squared_error(y_val, val_pred))
